    ...
    <div className="min-h-screen bg-background">
      <main className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
          <h1 className="font-pixel text-3xl md:text-4xl font-medium tracking-tighter text-foreground">
            AI Tools
          </h1>
          <div className="flex flex-row gap-2 md:gap-4 w-full md:w-auto">
            <div className="flex flex-col">
              <label className="font-pixel text-base mb-1 text-foreground">
                Category
              </label>
              <Select
            ...
            <div className="flex flex-col">
              ...
              <Select
                ...
                onValueChange={(label) =>
                  setSelectedPrice(
                    priceRanges.find((r) => r.label === label) || priceRanges[0]
                  )
                }
              >
                <SelectTrigger className="w-40 bg-white/80 border border-border rounded-lg shadow-card text-xs md:text-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-4">
          {filtered.length === 0 ? (
            <div className="col-span-full text-center text-muted-foreground font-pixel text-xl">
              No AI tools found.
            </div>
          ) : (
            filtered.map((card, idx) => (
              <ServiceCard key={idx} {...card} isSubscription={true} />
            ))
          )}
        </div>
      </main>
