...
const categoryIcons = {
  "iTunes Gift Cards (USD)": "/assets/icons/apple-store.svg",
};

const infoSections = [
  {
    title: "How to Redeem iTunes Gift Card",
    content: (
...
export default function ITunesGiftCard() {
  const [selectedItems, setSelectedItems] = useState<SelectedItem[]>([]);
  const [itgc, setItgc] = useState(null);
  ...
  useEffect(() => {
    async function fetchGiftCards() {
      try {
        ...
        // Find iTunes Gift Card (case-insensitive)
        const itunesGiftCard = products.find(
          (g) => g.title && g.title.toLowerCase() === "itunes gift card"
        );
        setItgc(itunesGiftCard);

        if (itunesGiftCard && Array.isArray(itunesGiftCard.priceList)) {
          // Group into iTunes Gift Cards (USD)
          const cards = [];
          itunesGiftCard.priceList.forEach((item) => {
            const [label, price, hot] = item.split("|");
            cards.push({ label, price: Number(price), hot: hot === "true" });
          });
          setPriceList([
            {
              title: "iTunes Gift Cards (USD)",
              categoryIcon: categoryIcons["iTunes Gift Cards (USD)"],
              items: cards,
            },
          ]);
        }

        // Get similar products excluding iTunes Gift Card
        setSimilar(
          giftCards.filter((g) => g.title.toLowerCase() !== "itunes").slice(0, 4)
        );
      } catch (err) {
        setItgc(null);
        setPriceList([]);
