...
const GameDetailsLayout: React.FC<GameDetailsLayoutProps> = ({
  title,
  image,
  priceList,
  infoSections,
            ...
            <Card className="mb-6 p-4">
              <div className="flex flex-col gap-6">
                {priceList.map((category, catIdx) => (
                  <div key={catIdx}>
                    <h3 className="font-pixel text-base text-primary mb-2 pl-1 opacity-80">{category.title}</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {category.items.map((item, itemIdx) => (
                        <Button
                          ...
                          type="button"
                          variant={
                            selected && selected.categoryIdx === catIdx && selected.itemIdx === itemIdx
                              ? "default"
                              : "outline"
                          }
                          className={`relative flex justify-between items-center font-sans text-base md:text-lg px-4 py-3 h-auto`}
                          onClick={() => setSelected && setSelected({ categoryIdx: catIdx, itemIdx })}
                        >
              ...
              <form onSubmit={onSubmit || ((e) => e.preventDefault())}>
                <div className="mb-4">
                  <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    {typeof zoneId !== "undefined" && setZoneId && (
                      <div className="flex-1">
                        <label
                          className="block font-pixel text-base text-primary mb-1"
                          htmlFor="zoneId"
