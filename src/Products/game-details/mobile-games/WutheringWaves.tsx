...
function groupPriceList(priceList) {
  ...
  return [
    ...
    { title: "Crystals Packages", categoryIcon: categoryIcons["Crystals Packages"], items: crystals },
  ];
}
...
const infoSections = [
  ...
  },
  {
    ...
    content: <p>Open Wuthering Waves, tap your avatar. Your Player ID is shown below your name.</p>,
  },
];

export default function WutheringWaves() {
  ...
  const product = mobileGames.find((p) => p.title === "Wuthering Waves");
  ...
  useEffect(() => {
    async function fetchProduct() {
      try {
        ...
        const wuthering = products.find((g) => g.title?.toLowerCase() === "wuthering waves");
        setGame(wuthering);

        if (wuthering && Array.isArray(wuthering.priceList)) setPriceList(groupPriceList(wuthering.priceList));

        setSimilar(mobileGames.filter((g) => g.title !== "Wuthering Waves").slice(0, 4));
      } catch {
        setGame(null);
        setPriceList([]);
        setSimilar([]);
