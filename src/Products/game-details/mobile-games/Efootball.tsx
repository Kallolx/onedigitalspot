...
const categoryIcons = {
  ...
  "Coins Packages": "/assets/icons/games/efootball.svg",
};
...
function groupPriceList(priceList) {
  ...
  return [
    ...
    { title: "Coins Packages", categoryIcon: categoryIcons["Coins Packages"], items: coins },
  ];
}
...
const infoSections = [
  ...
  },
  {
    ...
    content: <p>Open eFootball, tap your avatar. Your Player ID is shown below your name.</p>,
  },
];
...
export default function EFootball() {
  ...
  const product = mobileGames.find((p) => p.title === "eFootball");
  ...
  useEffect(() => {
    async function fetchProduct() {
      try {
        ...
        const football = products.find((g) => g.title?.toLowerCase() === "efootball");
        setGame(football);

        if (football && Array.isArray(football.priceList)) setPriceList(groupPriceList(football.priceList));

        setSimilar(mobileGames.filter((g) => g.title !== "eFootball").slice(0, 4));
      } catch {
        setGame(null);
        setPriceList([]);
        setSimilar([]);
